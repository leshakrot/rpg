...
{
    public class HerbSource : HarvestableSource
    {
        [Header("Настройки куста")]
        [SerializeField] private string harvestAnimationTrigger = "StartGathering";
        [SerializeField] private string stopAnimationTrigger = "StopGathering";
        ...
        protected override void StartHarvestAnimation()
        {
            if (currentPlayer != null)
            {
                ...
                if (playerAnimator != null && !string.IsNullOrEmpty(harvestAnimationTrigger))
                {
                    ...
                    if (HasAnimatorParameter(playerAnimator, "IsGathering"))
                    {
                        playerAnimator.SetBool("IsGathering", true);
                    }
                }
